...
def load_ghc_map() -> dict[str, str]:
    ...
    HLS_COMPAT_GHCS_FILE points at a JSON file (what the Nix build sets);
    HLS_COMPAT_GHCS may hold the same JSON inline (handy for tests).
    """
    path = os.environ.get("HLS_COMPAT_GHCS_FILE")
    raw = Path(path).read_text() if path else os.environ.get("HLS_COMPAT_GHCS")
    if not raw:
        return {}
